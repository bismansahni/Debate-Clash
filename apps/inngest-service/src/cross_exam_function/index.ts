...
import {
    CrossExamQuestionSchema,
    CrossExamAnswerSchema,
    CrossExamAnalysisSchema
...
export const crossExaminationFunction = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        console.log(`⚔️ Cross-Examination Round ${roundNum}: ${questioner.persona.name} questions ${respondent.persona.name}`);
...
Be objective but decisive. Someone won this exchange.`
        ...
        await step.run("detect-controversy", async () => {
            const moments = detectControversyMoments(
                { answers, analysis },
                respondent.persona.name
            );
        ...
        await step.run("store-results", async () => {
            enhancedDebateStore.setCrossExamRound(
                debateId,
                roundNum as 1 | 2,
                {
